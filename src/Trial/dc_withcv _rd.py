...
def load(path):
    ...
    if 'TEST_DATE' in df.columns:
        ...
        df = df.rename(columns={'DATEPRD': 'TEST_DATE'})
        df['TEST_DATE'] = pd.to_datetime(df['TEST_DATE'])
    else:
        raise KeyError("No date column found. Expected 'TEST_DATE' or 'DATEPRD'.")
    return df

def features(df, oil_col):
    '''
    ...
    df['1/logq']=1/np.log(df[oil_col].replace(0, np.nan))
    #rolling mean
    df['q_rolling_mean'] = df[oil_col].rolling(window=3, min_periods=1).mean()
    #dq/dt
    df['dq_dt'] = df[oil_col].diff() / df['t'].diff()
    #logq
    df['logq'] = np.log(df[oil_col].replace(0, np.nan))
    #lagged features only from previous time steps to avoid leakage
    df['q_lag1'] = df[oil_col].shift(1)
    df['q_lag2'] = df[oil_col].shift(2)
    df['q_lag3'] = df[oil_col].shift(3)
    # Replace zeros to avoid log/ratio issues
    df['q'] = df[oil_col].replace(0, np.nan)
    return df
...
def main():
    ...
    if 'WELL_NAME' in df.columns:
        well_col = 'WELL_NAME'
    elif 'WELL_BORE_CODE' in df.columns:
        well_col = 'WELL_BORE_CODE'
    else:
        raise KeyError("No well column found. Expected 'WELL_NAME' or 'WELL_BORE_CODE'.")

    if 'OIL' in df.columns:
        oil_col = 'OIL'
    elif 'BORE_OIL_VOL' in df.columns:
        oil_col = 'BORE_OIL_VOL'
    else:
        raise KeyError("No oil-rate column found. Expected 'OIL' or 'BORE_OIL_VOL'.")

    well_name='MAHONI-13BP1'
    if well_name not in set(df[well_col].dropna().unique()):
        fallback_well = df[well_col].dropna().iloc[0]
        print(f"Well '{well_name}' not found. Using '{fallback_well}' instead.")
        well_name = fallback_well

    df = df[df[well_col] == well_name].copy()
    #filter only from 2013 onwards
    df = df[df['TEST_DATE'] >= pd.to_datetime('2023-01-01')].copy()
    # Aggregate by date
    df = df.groupby('TEST_DATE', as_index=False)[oil_col].sum()
    df = df.sort_values('TEST_DATE').reset_index(drop=True)
    df = features(df, oil_col)
